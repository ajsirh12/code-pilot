...
**판단 기준**: 파일 산출물(코드, 문서, 디자인 등)이 예상되는 작업인가?

```
사용자 요청 분석
      ↓
파일 산출물이 예상되는 작업인가? (Claude 판단)
      ↓
  Yes → "관련 이슈가 있는지 확인할까요?"
           ├─ 있음 → 기존 이슈 연결, 브랜치 생성
           └─ 없음 → "이슈를 먼저 생성할까요?"
```

**특수 케이스**:
- 릴리즈/배포 요청 → "보안 감사 먼저 실행할까요?" (`security-auditor`)
- MR 생성 요청 → "관련 이슈를 연결할까요?" (`Closes #이슈번호`)

---

### 🟢 Post-work: 작업 완료 후 개입

**트리거 (100% 확실)**: Edit/Write/Task 도구 사용 완료

```
도구 사용 완료 (Edit/Write/Task)
      ↓
git status --porcelain 확인
      ↓
변경사항 있음?
      ↓
  Yes → "변경사항을 커밋할까요?" → /gl-commit
           ↓
        커밋 완료 후
           ↓
        "MR을 생성할까요?" → /gl-mr create
           ↓
        MR 생성 후 파이프라인 실패?
           ↓
          Yes → "디버깅을 도와드릴까요?" (pipeline-debugger)
```
...
프로젝트 초기 1회만 실행 (Pre-work/Post-work 불필요):

`/gl-bootstrap`, `/gl-group`, `/gl-labels`, `/gl-board`, `/gl-template`, `/gl-templates`, `/gl-webhook`, `/gl-variables`, `/gl-environments`, `/gl-runners`, `/gl-deploy-keys`, `/gl-tokens`, `/gl-members`
