...
describe('ConsoleFormatter', () => {
    describe('toString', () => {
        ...
        it('formats an UncaughtError', async (t) => {
            const error = new UncaughtError('Uncaught TypeError: Cannot read properties of undefined', undefined, '<mock target ID>');
            const result = (await ConsoleFormatter.from(error, { id: 4, fetchDetailedData: true })).toString();
            t.assert.snapshot?.(result);
        });
    });
    describe('toStringDetailed', () => {
        ...
        it('formats an UncaughtError with a stack trace', async (t) => {
            ...
            const error = new UncaughtError('Uncaught TypeError: Cannot read properties of undefined', undefined, '<mock target ID>');
            const result = (await ConsoleFormatter.from(error, {
                id: 7,
                resolvedStackTraceForTesting: stackTrace,
            })).toStringDetailed();
            t.assert.snapshot?.(result);
